document.addEventListener("DOMContentLoaded", function () {
    ...
    $("#button-submit-setting-change").on("click", function () {
        ...
        req.onload = function () {
            if (req.readyState === 4) {
                ...
                if (source.includes("status")) {
                    ...
                    if (obj["status"]) {
                        window.location = "http://localhost/exchange.cash/backend/login.php";
                    } else {
                        var error = obj["error"];
        ...
        req.open("POS", "http://localhost/exchange.cash/backend/tools.php?command=change_password", true);
